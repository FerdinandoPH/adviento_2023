    ...
    for i in range(nuevo_ini,nuevo_fin+1):
      if lineas[l][i].isdigit():
        for parte in lista_partes:
          if parte.linea==l and i in range(parte.ini,parte.fin) and parte.id not in (p.id for p in partes):
            partes.append(parte)
  return partes
with open("dia3.txt","r") as f:
    ...
    for j,caracter in enumerate(linea):
      if caracter.isdigit():
        ...
        if es_parte(lineas,i,inicio,j):
          num_partes+=1
          partes.append(Parte(num_partes,i,inicio,j))
        numero=""
        inicio=None
    if numero!="":
  ...
  print("Piezas:",end=" ")
  for parte in partes:
    print(lineas[parte.linea][parte.ini:parte.fin],end=",")
  print("\n")
    ...
    for j,caracter in enumerate(linea):
      if caracter=="*":
        ...
        if len(partes_adjuntas)==2:
          multi=1
          for parte in partes_adjuntas:
            multi*=int(lineas[parte.linea][parte.ini:parte.fin])
        print("Sumando",multi)
        suma+=multi
  print(suma)